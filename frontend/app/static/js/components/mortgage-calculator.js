...
(function () {
    ...
    window.calcMortgage = function () {
        ...
        // ---- affordability: monthly payment <= 25% of take-home ----------------
        const takeHome = parseFloat($('mort-takehome').value);
        const target25 = totalMonthly / 0.25; // required monthly take-home for <=25%
        let af = '';
        af += '<div class="readout-label">Affordability (25% rule)</div>';
        if (!isNaN(takeHome) && takeHome > 0) {
            const ratio = totalMonthly / takeHome;
            const ok = ratio <= 0.25;
            af += '<div class="calc-line"><span class="text-soft">Monthly payment</span>' + money(totalMonthly) + '</div>';
            af += '<div class="calc-line"><span class="text-soft">Your take-home</span>' + money(takeHome) + '</div>';
            af += '<div class="calc-line"><span class="text-soft">% of take-home</span><strong>' + (ratio * 100).toFixed(1) + '%</strong></div>';
            af += '<div class="calc-line" style="margin-top:6px"><span class="text-soft">' + (ok ? '✓ Within' : 'Over') + ' 25%</span><strong style="color:' + (ok ? 'var(--green, #22c55e)' : 'var(--red, #ef4444)') + '">' + (ok ? 'Affordable' : 'Too much') + '</strong></div>';
            if (!ok) {
